...
namespace Game2 {
    ...
    class Player : PhysicsObject {
        ...
        public void brandonUpdate(GamePadState gamePadState, KeyboardState keyboardState, double dt) {
           ...
           if (keyboardState.IsKeyDown(Keys.Right)) { //if the right key is down accelerate in +x direction
              if (vel.X < MAX_WALK_VEL) force.X = ACCEL;
           }

           if (keyboardState.IsKeyDown(Keys.Left)) { //if the left key is down accelerate is -x direction
           {
              if (vel.X < -1.0 * MAX_WALK_VEL) force.X = -1.0 * ACCEL;
           }

           //if neither key is held down we want it to stop moving
           if (keyboardState.IsKeyUp(Keys.Left) && keyboardState.IsKeyUp(Keys.Right)) {
              if (vel.X > DECCELL) { //if the velocity is sufficiently positive, DECCELL in units of velocity here
                 force.X = -1.0 * DECCELL;
              }
              else if (vel.X < -1.0 * DECCELL) { //if the velocity is sufficiently negative
                 force.X = DECCELL;
              }
              else { //velocity is sufficiently close to zero
                 vel.X = 0.0
              }
